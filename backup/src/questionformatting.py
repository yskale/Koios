...
def json_study_formatting(study):
    ...
    for study_no in range(len(study)):
        ...
        for questions in study[study_no]['questions']:
            ...
            try:
                json_formatted = json.loads(cleanned_questions)
                for i, question_data in enumerate(json_formatted):
                ...
                print("Exited study no",study[questions]['study_name'])
                count = count + 1
    return(count1,count)
file_path = f"/Users/ykale/Documents/Dev/koios/Koios/LLM_generated_questions/persona_1_questions.json"
